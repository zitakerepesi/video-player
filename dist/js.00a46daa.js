...
var interval;
var jump = 5;
var volume = 0.75; // TODO 1. rewind/forward +/- [1,2,3,4,5] seconds
...
function toggleVideo() {
  if ($video.paused) {
    ...
    $video.play();
    interval = (_interval = interval) !== null && _interval !== void 0 ? _interval : setInterval(updateProgressBar, 1000 / 30);
  } else {
    $video.pause();
...
function updateProgressBar() {
  ...
  $progress.style.right = "".concat(100 - progress, "%");
}

function renderButtons() {
  $buttons.innerHTML = "\n    <button class=\"video-toggle\">".concat($video.paused ? 'play' : 'pause', "</button>\n    <select class=\"rew-fwd-select\">\n      <option value=\"1\">1</option>\n      <option value=\"2\">2</option>\n      <option value=\"3\">3</option>\n      <option value=\"4\">4</option>\n      <option value=\"5\" selected>5</option>\n    </select>\n    <button class=\"rwd\">R</button>\n    <button class=\"fwd\">F</button>\n    <input type=\"range\" class=\"volume\" min=\"0\" max=\"100\" step=\"1\" value=\"").concat(volume * 100, "\">\n  ");
}
...
document.body.addEventListener('change', function (e) {
  var isSelect = e.target.classList.contains('rew-fwd-select');
  if (isSelect) jump = parseInt(e.target.value);
});
document.body.addEventListener('input', function (e) {
  var isVolume = e.target.classList.contains('volume');
